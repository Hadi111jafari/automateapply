...
| 2026-08-25 | P1 manual verification — automated pass | [~] | 26/26 API checks passed: signup validation + failed login + sign-in + invalid-token rejection; two-account isolation (profiles, applications create/list/mutate, resume rows, Storage object RLS blocked for other user and anonymous); jobs success (31 jobs, both sources) and empty states; route guards redirect all six workspace URLs; demo bypass works. lint/tsc/build all green. Blocked for the user: AI tailoring currently fails on every provider from this machine (OpenRouter ETIMEDOUT — intermittent network block; Together 401 invalid key; Gemini region-blocked "location not supported") and Confirm Email appears disabled so confirmation-redirect flow is untestable. Remaining truly-manual: email-confirmation link UX (if enabled later), desktop/mobile visual pass, provider key fixes. |
