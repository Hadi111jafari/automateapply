...
import { FileUp, LoaderCircle, RotateCcw, Send, Sparkles, Trash2, ZoomIn, ZoomOut } from 'lucide-react';
...
function ResumeWorkspace({ demo = false }: { demo?: boolean }) {
  ...
  useEffect(() => {
    ...
    }).catch((cause: unknown) => setError(cause instanceof Error ? cause.message : 'Could not load your resume workspace.'))
  ...
  const saveResume = async () => {
    ...
    try {
      if (file) {
        const body = new FormData(); body.set('content', source); body.set('file', file);
  ...
  const removeResume = async () => {
    if (demo || !resumeId || !window.confirm('Delete this resume and its original private file?')) return;
    setSaving(true); setError(undefined);
    try { await apiFetch(`/api/resumes?id=${resumeId}`, { method: 'DELETE' }); const next = resumes.filter((item) => item.id !== resumeId); const nextId = next[0]?.id ?? ''; setResumes(next); setResumeId(nextId); setSource(next[0]?.content ?? ''); activateMaterial(nextId, jobId); setNotice('Resume deleted.'); }
